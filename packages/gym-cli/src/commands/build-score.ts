...
export function registerBuildScore(program: Command): void {
  program
    ...
    .option("--repo <path>", "path to the repo containing your implementation", process.cwd())
...
`.trim();
      ...
      await Promise.all([
        writeFile(path.join(FEEDBACK_DIR, `build-evaluation-${timestamp}.md`), evaluation),
        writeFile(path.join(FEEDBACK_DIR, "latest-build-evaluation.md"), evaluation),
      ]);
